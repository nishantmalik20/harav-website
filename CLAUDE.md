...
**Phase:** Phase 1 complete + integrations wired in TEST mode (Stripe, Resend, Supabase). Boutique flipped ON locally. Booking form REBUILT as a 4-step wizard with dynamic per-service consultation questionnaires (client's paper forms digitized) — bookings schema now FINAL. Next: run `supabase/migrations/0001_init.sql` in the Supabase SQL editor (+ create admin user) → booking flow goes fully live locally. Then Vercel link + LIVE keys + production `NEXT_PUBLIC_SITE_URL`.
...
- [ ] **Supabase tables NOT yet created** — migration `supabase/migrations/0001_init.sql` is now FINAL (bookings table includes `intake_form` / `intake` jsonb / `consented_at` for the consultation questionnaires) and ready to run in the SQL editor; bookings + newsletter writes will fail until then
